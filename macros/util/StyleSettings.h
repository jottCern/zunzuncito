// $Id: StyleSettings.h,v 1.22 2012/06/08 21:14:44 mschrode Exp $
...
namespace util {
  ...
  public:
    ...
    enum Style { Screen, Presentation, PresentationCOLZ, Note, PAS, JMEPaper};
    ...
    static Style style() {
      ...
      else if( mode == "JMEPaper" ) st = JMEPaper;
    ...
    static void setStyleJMEPaper() { setStyle(JMEPaper,true); }
    static void setStyleJMEPaperNoTitle() { setStyle(JMEPaper,false); }
  ...
  private:
	...
	if( spaceForTitle ) gStyle->SetTitleYOffset(1.25);
	else                gStyle->SetTitleYOffset(1.25);
      } else {
	// For the axis labels:
	gStyle->SetLabelFont(42,"XYZ");
	gStyle->SetLabelOffset(0.007,"XYZ");
	...
	if( spaceForTitle ) {
      ...
      else if( mode == JMEPaper ) std::cout << "CMS JME Paper" << std::flush;
