...
namespace util
{
  ...
  public:
    ...
    // -------------------------------------------------------------------------------------
    static void setCommonYRange(std::vector <TH1 *> histos, size_t nLabelLines, double logMin = -1.) {
      if(histos.size()>0){
	double min = 0.;
	double max = 0.;
	findYRange(histos.at(0),nLabelLines,min,max,logMin);
	for(unsigned int i = 1; i < histos.size(); i++) {
	  double minTmp = 0;
	  double maxTmp = 0;
	  findYRange(histos.at(i),nLabelLines,minTmp,maxTmp,logMin);
	  if( minTmp < min ) min = minTmp;
	  if( maxTmp > max ) max = maxTmp;
	}
	for(unsigned int i = 0; i < histos.size(); i++) {
	  histos.at(i)->GetYaxis()->SetRangeUser(min,max);
	}
      }
    }
    ...
    static void setYRange(TH1 *h, int nLabelLines, double logMin = -1.) {
      ...
      //      std::cout << "set range "<< min << " to " <<max <<std::endl;
      h->GetYaxis()->SetRangeUser(min,max);
    }
    ...
    // -------------------------------------------------------------------------------------
    static TH1 *createFrame(double xMin, double xMax, double yMin, double yMax, const TString &xTitle, const TString &yTitle) {
      ++COUNT_;
      TH1 *h = new TH1D("util::HistOps::hFrame"+toTString(COUNT_),"",1000,xMin,xMax);
      //      for(int xBin = 1; xBin <= h->GetNbinsX(); ++xBin) {
      //	h->SetBinContent(xBin,1.);
      //      }
      h->GetXaxis()->SetTitle(xTitle);
    ...
    // --------------------------------------------------
    static bool fitCoreWidth(const TH1* hist, double nSig, TF1* &gauss) {
      double rms = 0.;
      double rmsErr = 0.;
      double width = 0.;
      double widthErr = 0.;
    ...
    // --------------------------------------------------
    // normalize each bin of each histogram sucht that
    // sum (histos) bin,i = 1
    // , i.e. get composition thstack
    // --------------------------------------------------
    static void normalizeEachBinCompositionTHStackHistogram(THStack* hStack) {


      TIter iterToFirstOnly(hStack->GetHists());
      TH1D* first=(TH1D*)iterToFirstOnly();
      Int_t nBins = first->GetNbinsX();

      for(Int_t bins_i = 0; bins_i < nBins+1; bins_i++){
	Double_t IntegralBin=0;
	TIter next(hStack->GetHists());
	TObject *obj;// =  next();
	while ((obj =  next())){
	  TH1D* temp=(TH1D*)obj;
	  IntegralBin+=temp->GetBinContent(bins_i);
	}
	IntegralBin/=100;//normalize to 100%
	
	TIter nextTwo(hStack->GetHists());
	while ((obj =  nextTwo())){
	  TH1D* temp=(TH1D*)obj;
	  temp->SetBinContent(bins_i,temp->GetBinContent(bins_i)/IntegralBin);
	  temp->SetBinError(bins_i,temp->GetBinError(bins_i)/IntegralBin);
	}
